import requests
import allure
from date.urls import changing_data
from date.generate_unique_user import generate_unique_data


class TestChangingUserData:

    @allure.title('Изменение данных name пользователя с авторизацией')
    def test_with_authorization_change_name(self, auth_token_and_random_user_data):
    ...
    def test_with_authorization_change_email(self, auth_token_and_random_user_data):
        headers = auth_token_and_random_user_data[0]
        user_data = auth_token_and_random_user_data[1]
    ...
    def test_without_authorization(self, payload_registered_user):
